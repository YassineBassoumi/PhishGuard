import React, { useState, useEffect } from 'react';
import './GmailAuth.css';

const GmailAuth = ({ onAuthenticated }) => {
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);

    useEffect(() => {
        // Check if we're returning from OAuth callback
        const urlParams = new URLSearchParams(window.location.search);
        const authSuccess = urlParams.get('auth');
        const credsEncoded = urlParams.get('creds');
        
        if (authSuccess === 'success' && credsEncoded) {
            setLoading(true);
            
            try {
                // Decode credentials from URL
                const credsJson = atob(credsEncoded);
                const credentials = JSON.parse(credsJson);
                
                // Store in localStorage for future use
                localStorage.setItem('gmail_credentials', JSON.stringify(credentials));
                
                // Clean up URL
                window.history.replaceState({}, document.title, window.location.pathname);
                
                // Notify parent
                onAuthenticated(credentials);
                setLoading(false);
            } catch (err) {
                setError('Failed to process credentials: ' + err.message);
                setLoading(false);
            }
        } else if (authSuccess === 'error') {
            const errorMsg = urlParams.get('message') || 'Unknown error';
            setError('Authentication failed: ' + decodeURIComponent(errorMsg));
        }
    }, [onAuthenticated]);

    const handleLogin = async () => {
        setLoading(true);
        setError(null);

        try {
            const response = await fetch('http://localhost:8000/api/gmail/auth');
            
            if (!response.ok) {
                throw new Error('Failed to initiate authentication');
            }

            const data = await response.json();
            
            // Redirect to Google OAuth
            window.location.href = data.auth_url;
        } catch (err) {
            setError(err.message);
            setLoading(false);
        }
    };

    if (loading) {
        return (
            <div className="gmail-auth-container">
                <div className="glass-card auth-card">
                    <div className="spinner"></div>
                    <p>Authentification en cours...</p>
                </div>
            </div>
        );
    }

    return (
        <div className="gmail-auth-container fade-in-up">
            <div className="glass-card auth-card">
                <div className="auth-icon">
                    <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path d="M3 8L10.89 13.26C11.25 13.48 11.75 13.48 12.11 13.26L20 8M5 19H19C20.1 19 21 18.1 21 17V7C21 5.9 20.1 5 19 5H5C3.9 5 3 5.9 3 7V17C3 18.1 3.9 19 5 19Z" strokeWidth="2" strokeLinecap="round" />
                    </svg>
                </div>
                
                <h2 className="auth-title">Connectez votre Gmail</h2>
                <p className="auth-description">
                    Connectez-vous avec votre compte Google pour analyser vos emails directement depuis votre boîte de réception.
                </p>

                {error && (
                    <div className="error-message">
                        <p>{error}</p>
                    </div>
                )}

                <button 
                    className="btn btn-primary btn-gmail"
                    onClick={handleLogin}
                    disabled={loading}
                >
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                        <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                        <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                        <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                    </svg>
                    Se connecter avec Google
                </button>

                <p className="auth-note">
                    Nous n'accédons qu'à vos emails en lecture seule. Vos données restent privées et sécurisées.
                </p>
            </div>
        </div>
    );
};

export default GmailAuth;
